...
//extern "C" __EXPORT int auv_control_main(int argc, char *argv[]);
//
//
////define: ................
//
...
private:
  ...
  float _pressure_zero_level;

  int _lamp_intensity_pwm;
  int _changeStateManual2Auto;
...
AUVControl::~AUVControl()
{
  ...
  auv_control::g_control = nullptr;
}


void AUVControl::ForceMoment2Throttle(double Force[3], double Moment[3], double & throttle_0,
                                                                                 double & throttle_1,
                                                                                 double & throttle_2,
                                                                                 double & throttle_3,
                                                                                 double & throttle_4,
...
int AUVControl::pwm_lookup_table(double throttle )
{
...
float AUVControl::joystick_deadband(float joystick_value, float joystick_threshold)
{
...
void
AUVControl::vehicle_rates_setpoint_poll()
{
...
void
AUVControl::depth_estimate(float dt)
{
    ...
    float pressure_zero_level_rate =    -0.5f* _position_sp.z;
...
void AUVControl::control_lamp(float dt)
{
  ...
  if ((_printing_time%100 ==0)&&(print_in_control_lamp)) {    
    ...
    //PX4_INFO("Debug control _________________ lamp: %1.6f ", (double)_v_rates_sp.yaw );
           ...
           lamp_intensity_rate = -60.0f*_v_rates_sp.yaw;  //50.0f is a proportional coefficient
...
Vector<3> AUVControl::sat3Function(Vector<3> x, float delta_x)
{
  for (int i=0; i < 3; ++i){
    if (x(i) > delta_x) {
       x(i) = delta_x;
...
void
AUVControl::control_att(float dt)
{       
  ...
  float omega_d = 0.0f;                  //Should be input from joystick
  //max yaw_angular_velocity = pi/12
  omega_d    = -4.0f*0.2617f*joystick_deadband(trung_gian_3 ,0.2f);  
...
void AUVControl::inner_loop_control(float dt)
{
  ...
  */
  ...
  _Gamma_c_x_inner = 1.0f*Gamma_C(0);
...
void
AUVControl::task_main_trampoline(int argc, char *argv[])
{
  auv_control::g_control->task_main();
}

int
AUVControl::task_main()
{
        ...
        else
        {
          if ((_printing_time%100 ==0) && true) {    
            ...
            */
            ...
            //Lam giam gia tri luc va moment dieu khien khi chuyen trang thai tu manual2Auto
            if (_changeStateManual2Auto < 250){
              float he_so_ty_le = float(_changeStateManual2Auto)/float(250.0);
              Force[0]  =  he_so_ty_le*_v_force_sp.x;
              Force[1]  =  he_so_ty_le*_v_force_sp.y;
              Force[2]  =  he_so_ty_le*_v_force_sp.z;           
              Moment[0] =  he_so_ty_le*_Gamma_c_x_inner;
              Moment[1] =  he_so_ty_le*_Gamma_c_y_inner;   
              Moment[2] =  he_so_ty_le*_Gamma_c_z_inner;
            } else{
              _changeStateManual2Auto = 251;
              Force[0]  =  _v_force_sp.x;
              Force[1]  =  _v_force_sp.y;
              Force[2]  =  _v_force_sp.z;           
              Moment[0] =  _Gamma_c_x_inner;
...
AUVControl::start()
{
  ...
  /* start the task */
  _control_task = px4_task_spawn_cmd("auv_control",
             SCHED_DEFAULT,
             SCHED_PRIORITY_MAX - 5,
             2500,
             (px4_main_t)&AUVControl::task_main_trampoline,
             nullptr);
...
int auv_control_main(int argc, char *argv[])
{
	if (argc < 2) {
        warnx("usage: auv_control {start|stop|status}");
		return 1;
	}

	if (!strcmp(argv[1], "start")) {

        if (auv_control::g_control != nullptr) {
			warnx("already running");
			return 1;
		}

        auv_control::g_control = new AUVControl;
        ...
        if (OK != auv_control::g_control->start()) {
            delete auv_control::g_control;
            auv_control::g_control = nullptr;
			warnx("start failed");
			return 1;
		}
